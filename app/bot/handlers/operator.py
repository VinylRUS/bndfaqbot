...
from app.bot.states.ticket_states import OperatorReply
from app.bot.keyboards.common import get_main_menu_operator, get_cancel_keyboard
...
from app.utils.permissions import can_handle_tickets
...
async def close_ticket(callback: CallbackQuery) -> None:
    ...
    from aiogram import Bot
    from app.config.settings import get_settings
    from app.bot.keyboards.user import get_rating_keyboard
    settings = get_settings()
    bot = Bot(token=settings.bot_token)
    try:
        await bot.send_message(
